                    ...
                    <div className="flex flex-col h-full justify-between mt-1">
                        <div className="flex items-center gap-3">
                            <div className="relative w-14 h-14 rounded-xl bg-gradient-to-br from-violet-500 to-fuchsia-600 shadow-lg flex items-center justify-center flex-shrink-0 overflow-hidden select-none">
                                <Disc3
                                    className={`w-8 h-8 text-white/90 transition-transform ${audioPlaying ? "animate-[spin_4s_linear_infinite]" : ""}`}
                                />
                            </div>
                            <div className="flex flex-col min-w-0 flex-1 overflow-hidden">
                                {audioTrack.title.length > 22 ? (
                                    <div className="w-full overflow-hidden whitespace-nowrap relative after:absolute after:right-0 after:top-0 after:h-full after:w-4 after:bg-gradient-to-l after:from-white dark:after:from-[#0A101A]/10 after:to-transparent">
                                        <div className="animate-news-ticker">
                                            <span className="text-sm font-bold text-slate-800 dark:text-white pr-8">
                                                {audioTrack.title}
                                            </span>
                                            <span className="text-sm font-bold text-slate-800 dark:text-white pr-8">
                                                {audioTrack.title}
                                ...
                                <span className="text-[9px] font-semibold text-slate-400 dark:text-violet-400/30 uppercase tracking-widest mt-1.5 flex items-center gap-1.5">
                                    <span
                                        className={`w-1.5 h-1.5 rounded-full ${audioPlaying ? "bg-emerald-500 animate-pulse" : "bg-slate-400/60"}`}
                                    ></span>
                        ...
                        <div className="flex items-center justify-between gap-2 mt-auto pt-4 pb-1 w-full px-1">
                            <button
                                onClick={toggleLoop}
                                className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all cursor-pointer border ${isLooping ? "text-violet-600 dark:text-violet-400 bg-violet-500/10 border-violet-500/30 shadow-[0_0_12px_rgba(139,92,246,0.2)]" : "text-slate-400 hover:text-slate-600 dark:text-violet-400/40 dark:hover:text-violet-400/70 border-violet-500/30 dark:border-violet-500/10 bg-slate-50 dark:bg-slate-900/20"}`}
                                title={isLooping ? "Disattiva Loop" : "Attiva Loop"}
                            >
                                <Repeat
                                    className={`w-4 h-4 ${isLooping ? "stroke-[2.5px]" : ""}`}
                                />
                            </button>
                            <button
                                onClick={skipBackward}
                                className="p-2 text-slate-400 hover:text-violet-600 dark:text-violet-400/60 dark:hover:text-violet-300 transition-colors cursor-pointer"
                            >
                                <SkipBack className="w-5 h-5 fill-current" />
                            </button>
                            <button
                                onClick={toggleAudioPlayback}
                                className="w-12 h-12 rounded-full bg-violet-600 text-white flex items-center justify-center shadow-[0_0_20px_rgba(139,92,246,0.3)] hover:bg-violet-700 hover:scale-105 transition-all duration-300 cursor-pointer flex-shrink-0"
                            >
                                {audioPlaying ? (
